...
async def startup_health_check(timeout: float=120.0) -> bool:
    try:
...
async def health_check():
    try:
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"status": ServiceStatus.UNHEALTHY, "error": str(e)},
                            )
